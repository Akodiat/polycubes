...
int main(int argc, char** argv) {
    const int colBits = 5;
    const int rotBits = 2;
    int nColors = pow(2, colBits); // 32;
    int nRotations = pow(2, rotBits); // 4;
    int maxRules = 5;
    if (argc > 1) {
        nColors = std::stoi(argv[1]);
    }
    if (argc > 2) {
        maxRules = std::stoi(argv[2]);
    }
    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
    int hex = 16;
    std::mt19937 r(seed);
    std::uniform_int_distribution<size_t> signDist(0, 1);
    std::uniform_int_distribution<size_t> colorDist(0, nColors-1);
    std::uniform_int_distribution<size_t> rotDist(0, nRotations-1);
    std::uniform_int_distribution<size_t> nRuleDist(1, maxRules);
    int nRules = nRuleDist(r);
    while (nRules --) {
        for(int i=0; i<6; i++) {
            int sign  = signDist(r);
