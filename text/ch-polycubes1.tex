...
Finally, by constraining the input space not to use the back and front patches, while orienting the remaining patches to point towards the front direction, the output space will correspond to 2D polyominoes.
...
%Finally, it is important to note that some identical polycubes might still have been separately counted if one of the rules found for them has an undetected likelihood of assembling into a different shape. While this may cause some polycubes to appear slightly less frequent than they are, this should not have any considerable effect on the results.
