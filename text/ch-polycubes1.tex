...
The assembly is repeated \(n_{times}\) times (default 100), and the outputs are compared for equality (allowing rotation) in order to determine if the rule is \emph{deterministic}. Note that this method is not rigourous, but it is enough to quickly detect a large part of the undeterministic assemblies. Figure~\ref{fig:UND}.b) shows a non-deterministic structure, where the blue ``neck'' species can bind to itself. Thus, the output depends on how many cubes from the blue species bind before a green species cube stops the growth. Both species are equally likely to bind, so the probability of assembling a neck with \(l\) blue cubes is \(2^{-l}\).
...
Having now understood the assembly model, let us use it to explore how the space of input rules map onto the output shapes. First of all, trying every input in a brute-force approach would unfeasible for most input spaces. For a 3D polycube space with \(\widetilde{K}_s\) species and \(\widetilde{K}_c\) colours, four possible patch orientations, and six patches per species, we get the following input space size, where \(I_{\widetilde{K}_s, \widetilde{K}_c}^{3D}\) is the set of all inputs:
\[
...
\begin{table}[h!]
    ...
    \begin{tabular}{|r||p{2cm}|c|c|c|c|}
        ...
        Number of samples       & \(10^9\) & \multicolumn{4}{|c|}{\(10^6\)} \\ \hline
...
With the input space investigated, we can also check how much of the output space we managed to sample. Figure~\ref{fig:ref_inputdistr} shows that most of the valid rules assembled into smaller shapes, with over 60 per cent (27 million) being 1-mers (note that the y-axis is logarithmic).
...
For the larger sizes, we again see the bias toward shapes with a multiple of four tiles. This shows that not only do some polyomino sizes have more input rules assembling them, but there is also larger number of unique polyomino shapes found for those sizes. The OEIS (On-line Encyclopedia of Integer Sequences) data shows no such bias in the actual size counts. Following the earlier argument, that few additional species are required to extend a shape with a multiple of four tiles, those tiles can be placed in different (still symmetric) configurations and create different shapes. Larger shapes leave room for more such permutations, leading to the increasing bias seen in the figure.
...
In general, the seeded assembly mode is expected to generate more valid rules than when the seed is random, as Figure~\ref{fig:valid_proportion} shows. 

An input rule with an undetermined seed can assemble different shapes even if a subset of cubes assembled deterministically and will be deemed non-deterministic, while the same rule would be considered valid for seeded assembly if the seed is chosen from a deterministic subset. With the third dimension, there are more degrees of freedom, i.e. more orientations and more patches per cube to attach to, leading to the significantly lower number of valid 3D rules seen in the figure.
...
% Andrew: Explain more clearly? Can this be tested?
